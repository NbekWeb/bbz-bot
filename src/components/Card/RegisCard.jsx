...
const Progress = ({ data = {} }) => {
  ...
  const success = Number(data?.done) || 0
  const progress = Number(data?.progress) || 0

  // Calculate widths dynamically
  const successWidth = total === 0 ? '50%' : `${(success * 100) / total}%`
  const progressWidth = total === 0 ? '50%' : `${(progress * 100) / total}%`
